...
LineChart, AreaChart, BarChart (grouped + `stacked`), ScatterPlot, BubbleChart, QuadrantChart, PieChart (+ donut via `innerRadius`), Histogram, RadarChart, TreemapChart (flat, grouped via `group`, or nested via `childrenKey`), SunburstChart (hierarchical radial with drill-down), WaterfallChart, SankeyDiagram, MekkoChart, ButterflyChart, CandlestickChart (OHLC, `xScaleType: 'band'` on the stringified date accessor), FunnelChart (continuously-tapering conversion funnel, self-contained), GaugeChart (speedometer/KPI dial, self-contained, single-value). The treemap's squarify tiling and sunburst's angle partitioning live in `core/treemap.ts` and `core/sunburst.ts` — implemented in-house (no `d3-hierarchy`), the same way `core/ticks.ts` replaced `d3-axis`. The gauge's needle/tick trig and threshold-band angle math live in `core/gauge.ts`, built on `core/shapes.ts`'s `arcPath()` (a standalone `d3-shape` `arc()` wrapper, not routed through `d3.pie()`) — hand-rolled the same way.
